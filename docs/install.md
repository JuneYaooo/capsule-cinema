...
| MiniMax H3 2K 视频 | `MINIMAX_API_KEY`；可选 `MINIMAX_VIDEO_BASE_URL`、`MINIMAX_VIDEO_MODEL`、`MINIMAX_VIDEO_TIMEOUT_SECONDS` |
...
MiniMax H3 使用官方 V2 接口，当前只支持 2K。默认完整视频仍用 Seedance 720p；只有明确要求 H3、Hailuo-03、2K、高分辨率，或显式传 `--video_engine minimax-h3` 时才选择 H3。H3 结果链接有时效，适配器会立即下载到本地；高分辨率路线失败时不会静默降级到 720p。
