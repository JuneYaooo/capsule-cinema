...
def main() -> int:
    ...
    try:
        ...
        storyboard_path, _, scenes = _locked_storyboard(params, output, expected_scenes=6)
        ...
        if args.dry_run:
            report = {
                ...
                "preflight": preflight["felt_asmr"],
            }
            path = output / "reports" / "run_notes.json"
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
            (output / "artifact_manifest.json").write_text(
                json.dumps({"dry_run": True, "deliverable": False, "artifacts": [{"path": str(path), "category": "qa", "title": "Felt exact-route preflight"}]}, ensure_ascii=False, indent=2) + "\n",
                encoding="utf-8",
            )
            print(json.dumps(report, ensure_ascii=False))
            return 0
        result = execute_felt_asmr(args.topic, params, output)
