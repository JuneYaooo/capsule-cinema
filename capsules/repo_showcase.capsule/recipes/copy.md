...

{
  "ai_flavored_template_copy_policy": {
    ...
    "required": true,
    "rewrite_rules": [
      "标题和底部卡片优先直接写具体用途：输入什么、检查什么、输出什么、下一步怎么做。",
  ...
  "copy_rules": [
    ...
    "做 repo/tool/skill 视频时，核心不是复述 README 或罗列数字，而是伯乐式理解项目：必要时深入源码、脚本、配置、示例和截图，把项目到底是什么、能帮目标用户完成什么、为什么值得停留/收藏，翻译成用户能立刻理解的标题和画面。",
    ...
    "开源信息有证据时只写“开源免费”；公开视频不要默认写“商用可用”，不要写 MIT 或其他开源协议名，除非用户明确要求授权细节。",
    ...
    "短视频信息必须层层递进：每一页只推进一个新信息，上一页提出疑问，下一页回答或推进；默认按问题定位、输入或素材、机制展开、证据证明、输出和下一步动作组织。胶囊只写通用结构，不把单条视频、单个项目或单个行业的具体术语沉淀为默认规则。",
...

{
  ...
  "bottom_copy": {
    ...
    "good_example_shapes": [
      ...
      "不是提示词，是触发链 -> brainstorming/plans/TDD/review -> 每个任务先查该用哪种 skill -> 覆盖多种 Code Agent CLI -> 争议点是值不值这些上下文",
      "高星标不是因为它省 Token -> 它把 Agent 从直接开写拽回来 -> 耗上下文，压返工率 -> 真实项目要稳，不是只要快"
    ],
  ...
  "memory_assets": {
    "asset_inventory": [
      ...
      "contrast: 不是 A 而是 B、旧做法 vs 新路径、普通工具 vs 独特机制",
