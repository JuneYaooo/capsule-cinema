...
W, H = 1080, 1440
MIDDLE_PANEL_BOX = (68, 356, 1012, 934)
MIDDLE_TITLE_Y = 362
...
MIN_BOTTOM_LINE_VISIBLE_CHARS = 12
...
def preflight_fail(message: str) -> None:
    raise SystemExit(f"repo_showcase preflight failed: {message}")
...
def load_source_asset_manifest(profile: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not manifest_path_value:
        preflight_fail("source_asset_manifest is required for approved repo_showcase renders")
...
def validate_actual_source_manifest(profile: dict[str, Any], scenes: list[dict[str, Any]]) -> None:
    ...
    for index, item in enumerate(manifest, start=1):
        ...
        if not str(item.get("capture_method") or "").strip():
            preflight_fail(f"source_asset_manifest[{asset_id}] missing capture_method")
        if not str(item.get("source_url_or_repo_path") or "").strip():
...
def validate_repo_showcase_profile(profile: dict[str, Any]) -> None:
    scenes = profile.get("scenes") or []
    if not scenes:
        preflight_fail("profile must include scenes")
    if len(scenes) > 5:
        preflight_fail("short_silent_repo_showcase supports at most 5 scenes")
...
def render_video(profile: dict[str, Any], output_dir: Path, root: Path, params_path: Path, topic: str) -> dict[str, Any]:
    ...
    validate_repo_showcase_profile(profile)
