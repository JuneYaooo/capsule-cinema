...
Commands assume only that the runtime project provides the wrappers. Configure paths explicitly; do not infer artifact locations from the skill package path.
...
VIDEO_ARTIFACT_ROOT="${VIDEO_ARTIFACT_ROOT:-$PROJECT_ROOT/artifacts/video_runs}"
RUN_ROOT="${RUN_ROOT:-$VIDEO_ARTIFACT_ROOT/production/$(date +%Y%m%d_%H%M%S)_topic}"
```
...
```bash
test -f "$VIDEO_WRAPPER_ROOT/run_action_transfer.py" && \
  python "$VIDEO_WRAPPER_ROOT/run_action_transfer.py" \
...
There is no portable executor wrapper in every checkout. Use this JSON shape for planning, capsule records, or managed runtimes that explicitly provide `run_executor.py`; otherwise execute scene steps with `run_tool.py` and assemble with the available video-agent scripts.
...
```bash
if [ -f "$VIDEO_WRAPPER_ROOT/run_executor.py" ]; then
  python "$VIDEO_WRAPPER_ROOT/run_executor.py" \
