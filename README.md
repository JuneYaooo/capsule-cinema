...
AI 视频创作工厂：把一次跑通的视频，沉淀成可以复用的视频配方。

你可以把胶囊理解成打包好的视频配方。它保存的不是某条成片，而是一类视频可复用的做法：怎么接需求、怎么写分镜、怎么定风格、怎么配音和剪辑、怎么检查质量，以及哪些坑不要再踩。

你给目标、素材和偏好，AI 选择合适的视频配方或通用流程，完成分镜、画面、视频、配音、BGM、字幕、拼接、QA 和经验沉淀。更准确地说，它是一条给持续做内容的人准备的创作流水线：把选题、风格、资产和质量标准沉淀下来，后面继续复用。
...
| 我想持续做自媒体 | 把选题、脚本、资产、返工记录和发布检查放进同一条流程，减少每条视频从零开始 |
| 我关心能不能发布 | 成片后生成本地 QA 报告、质量评分、修复计划和发布检查点 |
| 我有专门场景 | 使用内置视频配方；动作迁移、数字人口播、角色 MV 走专用能力路线 |
...
Capsule Cinema 内置了一批初始视频配方，覆盖项目展示、电商商品、艺术动态短片、治愈 ASMR、剧情口播、国风讲解等常见视频场景。

这些配方不是为了覆盖所有场景，而是给你一批可拆开的种子案例。你可以从里面看到一套视频配方应该怎么组织：适用边界、输入要求、分镜结构、视觉风格、音频策略、剪辑节奏、质量规则和已知坑。你不需要理解内部结构，只要点名配方并说明目标。
...
需要接入新渠道、新工具或新视频配方时，也可以直接用自然语言说明接口文档、目标流程和示例交互。

- “帮我新增一个 `<工具/渠道名>`，接口文档如下：`<粘贴文档>`。请注册完整流程、补测试，并写一个简单用户示例。”
- “帮我安装或启用 `<配方名>` 视频配方。README 里不用展示样片，只写用户怎么用自然语言点名它。”
