...
先通过真实浏览器打开项目 README，截取能够支撑文案的页面区域，再按配方完成视频。
这次不要调用 AI 视频、TTS 或图片生成工具；如果需要 BGM，只使用我提供或已经确认授权的本地音乐。
完成后把成片、封面和 QA 结果一起交付给我。
```

建议同时给 Agent 安装一个浏览器 CLI Skill，让它能够打开网页、定位 README 区域并直接截图。这样通常只需要本地 Python、浏览器和 FFmpeg，不会产生 AI 图片、视频或语音生成消耗。
