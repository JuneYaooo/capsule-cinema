...
| 我有一条参考视频想复用套路 | 分析本地视频的镜头节奏、文案结构、视觉风格、音频策略和质量规则，先生成胶囊草稿，再确认是否写入配方 |
...
| `CARD.md` / `index.md` | 给使用者和 AI 助手看的入口，说明这个配方适合什么、不适合什么 |
...
保存时可以创建新配方，也可以把这次更好的改动沉淀回已有配方。要分享给别人时，先整理成可分享版本，再检查敏感信息和不可分发素材。

如果你手上已经有一条想学习的参考视频，也可以先让 AI 分析视频本身，拆出镜头段落、节奏、文案、画面运动、音频策略和质量规则，生成一个胶囊草稿。草稿确认后再写入配方；源视频默认只是分析依据，不会直接变成每期必用素材。需要把源视频放进配方时，应标成 `reference_only`，只作为风格参考。
...
固定素材分两种语义：`reuse=always` 表示每期必用，例如固定 BGM；`reference_only` 表示只作风格参考，每期重做。保存配方时会检查这条边界，避免把风格参考误当成每期固定素材。
...
| 想做什么 | 对 AI 这样说 |
|----------|--------------|
| 沉淀回已有配方 | “这次是在 `<已有配方名>` 基础上做出来的，请把更好的改动沉淀回这个配方。” |
| 做分享版本 | “请把这个配方整理成可分享版本，先检查敏感信息和不可分发素材。” |
| 参考视频生成胶囊草稿 | “请分析这个本地参考视频 `<视频路径>`，拆出可复用的结构、风格、节奏、文案和质量规则，先生成 `<配方名>` 的胶囊草稿，不要直接写入配方。” |
| 确认后写入配方 | “这个胶囊草稿我确认了，请写入配方 `<配方名>`。源视频只作为风格参考，不要当成每期固定素材。” |
| 新增工具渠道 | “帮我新增一个 `<工具/渠道名>`，接口文档如下：`<粘贴文档>`。请把它接入 Capsule Cinema，并写一个简单用户示例。” |
| 定义工具说法 | “这个工具以后我想这样调用：`<一句用户示例>`。请按这个交互方式接入。” |
| 指定工具优先级 | “我想让 `<配方名>` 优先使用这个工具。如果不可用，请列出可替代工具，不要静默降级。” |
