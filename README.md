...
装好后列出内置配方和当前可用渠道，不要调用计费 API。
```

Agent 会选择当前环境的安装位置、安装 Python 依赖并检查 FFmpeg。升级已有安装时会保留 `.env`、本地渠道、自建配方和历史输出。完整说明见 [安装指南](docs/install.md)。
...
保存时，Agent 会区分栏目固定方法和本期变量。人物事实、价格、标题、旁白、临时素材、客户资料、密钥、远程临时 URL、绝对路径和运行产物不会进入可分享配方。
...
配方更新会先检查冲突和内容边界，验证失败时不会留下半更新状态。同名配方安装前需要版本或差异确认。
...
运行 `python3.12 scripts/capsule.py list` 可以查看当前安装中的配方；用 `show` 和 `doctor` 可以查看输入要求、执行方式、所需渠道和诊断结果。命令和目录中的 `capsule` 是兼容名称，对外统一称为视频配方。
...
<img src="docs/assets/readme-capsule-anatomy.svg" width="100%" alt="视频配方的包结构">
...
个人渠道默认放进 Git 忽略的本地覆盖层，密钥不会写入代码或公共配方。公共渠道贡献需要同步适配器、注册表、能力标签、环境变量白名单、测试和 QA。详细规则见 [自定义工具说明](lib/custom_tools/README.md)。
...
Capsule Cinema 是一个本地 Skills 项目。Agent 读取 `skill.md`、`references/`、`capsules/*.capsule/` 和工具注册表，通过脚本入口完成规划、生成、剪辑、QA、修复和配方生命周期操作。`capsules/*.capsule/` 是运行时兼容路径，不影响用户侧的「视频配方」命名。
...
- [视频配方包格式](references/capsule-package-format.md)
