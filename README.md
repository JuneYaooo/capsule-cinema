...
> 这是「渠道名」的 API 文档：「链接或本地文档」。帮我把它加到 Capsule Cinema，以后做视频时可以用这个渠道。
...
你不需要记任何命令。安装、做视频、接入新渠道和保存胶囊，都可以像平时聊天一样告诉 AI。
...
安装完成后，AI 会告诉你是否需要重启。重新打开后直接说想做什么即可。
...
```text
帮我用 Capsule Cinema 做一条 25 秒的竖屏视频：一只橘猫深夜经营路边摊，温暖治愈一点。先给我看看分镜，满意了再继续做。
```
...
| 目标 | 直接对 Agent 说 |
| --- | --- |
| 使用指定胶囊 | “用电商商品展示胶囊，帮我做一条 20 秒的桌面收纳产品视频。” |
| 使用真实素材 | “用这些素材帮我剪一条短视频，加上字幕和背景音乐。” |
| 只做分镜 | “我先看看分镜，暂时不用做成视频。” |
| 局部返工 | “第 3 个镜头不太好看，只改这个，其他地方别动。” |
| 做数字人口播 / 动作模仿 / MV | “帮我做一条数字人口播视频。” / “参考这段舞蹈做一个动作视频。” / “用这首歌做一个 MV。” |

### 4. 把自己的 API 文档交给 AI，安装新渠道

把渠道的 API 文档链接或本地文件交给 AI 即可。文档越完整，接入通常越顺利。

复制下面的模板：

```text
这是「渠道名」的 API 文档：「粘贴链接，或告诉它本地文件在哪里」。
帮我把它加到 Capsule Cinema，以后做视频时可以用这个渠道。需要我配置什么再告诉我。
```
...
```text
这条视频我很满意，帮我把这套做法保存成「治愈系夜间小摊」胶囊，以后做同类视频继续用。
```

从参考视频蒸馏：

```text
参考这个视频，帮我整理成一个以后可以重复使用的视频胶囊。先告诉我你准备保存哪些东西。
```
...
一次偶然效果不应该立刻改写胶囊。你只需要用自然语言告诉 AI 这次学到了什么；AI 会判断它能否复用、检查是否与已有规则冲突，并在实际更新前请你确认。

```text
这次产品近景放 2 秒左右感觉更好，帮我看看要不要记到电商视频胶囊里。
```

冲突检查、经验泛化、安全更新和结构校验由 Agent 自动处理，不需要你在提示词里逐项指定。

分享和安装胶囊也可以只说人话：

```text
把「治愈系夜间小摊」胶囊打包给我，我想分享给别人。
```

```text
帮我安装这个胶囊包。如果已经有同名胶囊，先问我再处理。
```
