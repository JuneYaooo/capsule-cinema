#!/usr/bin/env python3
"""Render a source-grounded GitHub/repo showcase profile into a 3:4 video.
...
W, H = 1080, 1440
MIDDLE_PANEL_BOX = (68, 326, 1012, 934)
MIDDLE_TITLE_Y = 362
MIDDLE_TITLE_Y_WITH_VALUE = 352
MIDDLE_CONTENT_START = 458
MIDDLE_CONTENT_START_WITH_VALUE = 424
MIDDLE_CONTENT_BOTTOM = 912
BOTTOM_BOX_DEFAULTS = {
    True: (956, 1362),
    False: (990, 1340),
}
...
def make_bg(accent: str, *, show_safe_bands: bool = False) -> Image.Image:
    ...
    grid_bottom = H - 145 if show_safe_bands else H
    grid_line_bottom = H - 150 if show_safe_bands else H
    for y in range(86, grid_bottom, 56):
        gd.line((0, y, W, y), fill=(38, 82, 128, 52), width=1)
    for x in range(0, W, 86):
        gd.line((x, 70, x, grid_line_bottom), fill=(38, 82, 128, 45), width=1)
    for x in range(-260, W, 110):
        gd.line((x, 70, x + 360, grid_line_bottom), fill=(40, 150, 230, 45), width=1)
...
def top_title_y_for_profile(profile: dict[str, Any]) -> int:
    return int(profile.get("top_title_y", profile.get("top_title_y_preferred", 108)))
...
def top_subtitle_min_y_for_profile(profile: dict[str, Any]) -> int:
    return int(profile.get("top_subtitle_min_y", profile.get("top_subtitle_min_y_preferred", 270)))
...
def draw_title(draw: ImageDraw.ImageDraw, profile: dict[str, Any], scene: dict[str, Any]) -> None:
    ...
    if subtitle:
        ...
        draw_centered(draw, subtitle, max(top_subtitle_min_y_for_profile(profile), title_end + 16), subtitle_f, "#CFE7F7")
...
def image_boxes_for_content(content_y: int, image_count: int) -> list[tuple[int, int, int, int]]:
    if content_y <= 500:
        one_image_top = max(374, min(int(content_y), 458))
        two_image_top = max(398, min(int(content_y + 24), 482))
        if image_count <= 1:
            return [(104, one_image_top, 976, MIDDLE_CONTENT_BOTTOM)]
        if image_count <= 2:
            return [(104, two_image_top, 524, MIDDLE_CONTENT_BOTTOM), (556, two_image_top, 976, MIDDLE_CONTENT_BOTTOM)]
        return [
            (104, two_image_top, 524, 672),
            (556, two_image_top, 976, 672),
            (104, 706, 524, MIDDLE_CONTENT_BOTTOM),
            (556, 706, 976, MIDDLE_CONTENT_BOTTOM),
        ]
    top = min(max(content_y + 24, 548), 700)
    if image_count <= 1:
...
def should_show_middle_title(profile: dict[str, Any], scene: dict[str, Any]) -> bool:
    return bool(scene.get("show_middle_title", profile.get("show_middle_title", True)))
...
def middle_content_start_y(profile: dict[str, Any], scene: dict[str, Any], *, has_value_block: bool, show_title: bool) -> int:
    if not show_title:
        return int(scene.get("middle_content_start_no_title", profile.get("middle_content_start_no_title", 374)))
...
def draw_visual(canvas: Image.Image, profile: dict[str, Any], scene: dict[str, Any], progress: float = 0.0) -> None:
    ...
    accent = scene.get("accent") or profile.get("accent") or "#167A6F"
    rounded(draw, MIDDLE_PANEL_BOX, 24, "#FFFDF6", accent, 3)
...
def draw_bottom(canvas: Image.Image, profile: dict[str, Any], scene: dict[str, Any]) -> None:
    ...
    lines = list(scene.get("bottom_lines", []) or [])
    ...
    title_size = int(scene.get("bottom_title_font_size", profile.get("bottom_title_font_size", 34 if dense else 38)))
    ...
    title_size = fit_rich_font_size(
        draw,
        title,
        start=title_size,
        minimum=int(profile.get("bottom_title_font_size_min", 36)),
        max_w=int(profile.get("bottom_text_max_w", 870)),
        max_h=int(profile.get("bottom_title_max_h", 68)),
        base_bold=True,
    )
    title_y = y1 + int(scene.get("bottom_title_y_offset", profile.get("bottom_title_y_offset", 22 if dense else 28)))
    title_h = rich_line_height(draw, title, title_size, base_bold=True)
    body_y_offset = int(scene.get("bottom_body_y_offset", profile.get("bottom_body_y_offset", 76 if dense else 88)))
    draw_rich_centered(draw, title, title_y, title_size, "#FFFFFF", highlight, base_bold=True)
    ...
    body_top = max(
        y1 + body_y_offset,
        title_y + title_h + int(profile.get("bottom_title_body_gap_min", 24)),
    )
    ...
    if footer and foot_f and footer_box:
        rounded(draw, footer_box, 12, "#F6F3EA")
...
def render_frame(profile: dict[str, Any], scene: dict[str, Any], out: Path, *, progress: float = 0.0) -> None:
    canvas = make_bg(
        scene.get("accent") or profile.get("accent") or "#167A6F",
        show_safe_bands=bool(scene.get("show_safe_bands", profile.get("show_safe_bands", False))),
    )
    draw = ImageDraw.Draw(canvas)
...
def reject_spoken_profile_fields(profile: dict[str, Any]) -> None:
    for key in SPOKEN_PROFILE_FIELDS:
        ...
        if isinstance(value, str) and value.strip():
            raise SystemExit(f"repo_showcase is silent-only; remove {key} from the profile")
...
def render_video(profile: dict[str, Any], output_dir: Path, root: Path, params_path: Path, topic: str) -> dict[str, Any]:
    ...
    duration = float(profile.get("target_duration", 10))
