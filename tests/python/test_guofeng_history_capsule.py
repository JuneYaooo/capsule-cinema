...
class GuofengHistoricalExplainerCapsuleTest(unittest.TestCase):
    ...
    def test_capsule_requires_real_motion_video_not_static_zoompan_final(self):
        ...
        self.assertTrue(config["video_elements"]["fixed"]["static_zoompan_fallback_preview_only"])
        self.assertTrue(config["video_elements"]["fixed"]["require_real_motion_video_segments"])
    ...
    def test_capsule_requires_character_bible_and_reference_sheet_before_storyboard_images(self):
        ...
        self.assertNotIn("Yang Zhen", rules_text)
        self.assertNotIn("Wang Mi", rules_text)
        self.assertIn("recurring character", rules_text)
