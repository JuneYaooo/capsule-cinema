...
class ReleaseArtifactsTest(unittest.TestCase):
    ...
    def test_release_checkpoint_blocks_failed_capsule_gate_report(self):
        final_video = self.workspace / "release" / "final_video.mp4"
        final_video.write_text("video")
        self.write_json(
            self.workspace / "artifact_manifest.json",
        ...
        self.write_json(self.workspace / "qa" / "repair_plan.json", {"blocking": False})
        self.write_json(
            self.workspace / "qa" / "capsule_gate_report.json",
            {
                "schema": "capsule_cinema.capsule_gate_report.v1",
                "phase": "release",
                "ok": False,
                "status": "blocked",
                "blockers": ["bottom_title_not_visible_required"],
                "checks": [
                    {
                        "id": "bottom_title_not_visible_required",
                        "ok": False,
                        "severity": "blocker",
                        "detail": {"path": "scenes[0].bottom_title"},
                    }
                ],
            },
        )
        ...
        self.assertIn("capsule_gate:bottom_title_not_visible_required", payload["blockers"])
        checks = {item["id"]: item for item in payload["checks"]}
        self.assertFalse(checks["capsule_gates_passed"]["ok"])
        categories = {item["category"] for item in payload["artifacts"]}
        self.assertIn("capsule_gate_report", categories)

    def test_release_checkpoint_allows_passing_capsule_gate_report(self):
        final_video = self.workspace / "release" / "final_video.mp4"
        final_video.write_text("video")
        self.write_json(
            self.workspace / "artifact_manifest.json",
        ...
        self.write_json(
            self.workspace / "qa" / "capsule_gate_report.json",
            {
                ...
                "status": "pass",
                "blockers": [],
                "checks": [{"id": "fallback_generated_card_preview_only", "ok": True, "severity": "blocker"}],
            },
        )
        ...
        self.assertNotIn("capsule_gate:fallback_generated_card_preview_only", payload["blockers"])
        checks = {item["id"]: item for item in payload["checks"]}
