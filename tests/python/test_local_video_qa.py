...
class LocalVideoQATest(unittest.TestCase):
    ...
    def test_expect_audio_fails_when_audio_stream_is_silent(self):
        local_video_qa.probe_video = lambda _path: {
            "ok": True,
            "duration": 8.0,
            "width": 720,
            "height": 1280,
            "has_audio": True,
            "audio_loudness": {"ok": True, "max_volume_db": -91.0},
            "format": {},
        }
        self.write_manifest(
            [
        ...
        args = self.qa_args()
        args.require_prompts = False

        report = local_video_qa.run_qa(args)

        self.assertFalse(report["ok"])
        checks = {item["id"]: item for item in report["checks"]}
        self.assertFalse(checks["audio_not_silent"]["ok"])
        self.assertEqual(checks["audio_not_silent"]["actual_max_volume_db"], -91.0)
