...
class ImageGenerationEngineSelectionTest(unittest.TestCase):
    ...
    def test_gpt_image2_declares_generic_daydream_env_alternative(self):
        capabilities = yaml.safe_load((ROOT / "lib" / "config" / "tool_capabilities.yaml").read_text(encoding="utf-8"))
        capability = capabilities["tools"]["GptImage2Tool"]

        self.assertIn(
            ["GPT_IMAGE2_API_KEY", "GPT_IMAGE2_BASE_URL"],
            capability["requires_env_any"],
        )
...
class Seedream5TimeoutTest(unittest.TestCase):
    def test_gpt_image2_images_api_enforces_total_stream_timeout(self):
        ...
        with patch.dict(
            "os.environ",
            {
                ...
                "GPT_IMAGE2_IMAGES_TOTAL_TIMEOUT": "1",
                "GPT_IMAGE2_IMAGES_READ_TIMEOUT": "1",
            },
            clear=True,
        ), patch(
            ...
            "custom_tools.image_generation.seedream5_image_generator_tool.time.monotonic",
            side_effect=[0, 0.5, 2.0],
        ):
            with self.assertRaisesRegex(TimeoutError, "Images API 响应总超时"):
...
class RuntimeImageBatchTimeoutTest(unittest.TestCase):
    def test_gpt_image2_scene_generation_honors_concurrency_env(self):
        ...
        generator = ImageGenerator()
        generator._generate_single_scene = MagicMock(
            return_value={
                "scene_id": 0,
        ...
        with tempfile.TemporaryDirectory() as tmpdir, patch.dict(
            "os.environ",
            {"GPT_IMAGE2_IMAGE_CONCURRENCY": "2"},
            clear=False,
        ), patch(
            "src.runtime.general_video_crew.image_generator.ThreadPoolExecutor",
            CapturingThreadPoolExecutor,
        ):
            generator.generate_scene_images(
                storyboard=[{"scene_id": i, "image_prompt_chinese": f"场景{i}"} for i in range(3)],
