...
class LifeShakerOpeningRendererTest(unittest.TestCase):
    ...
    def test_empty_candidate_terms_stay_empty_without_episode_defaults(self):
        renderer = load_renderer()

        self.assertEqual(renderer.candidate_terms_for_display([]), [])
        self.assertEqual(renderer.DEFAULT_CANDIDATES, [])
        self.assertEqual(renderer.DEFAULT_SERIES_TITLE, "每天一个模拟人生")
