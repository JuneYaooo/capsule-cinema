...
def load_capsule_script():
    spec = importlib.util.spec_from_file_location("art_motion", SCRIPT_PATH)
...
class ArtFrameCaptionPromptTest(unittest.TestCase):
    ...
    def test_bgm_selection_can_use_local_audio_path(self):
        plan = self.script.decide_frame_plan("安静的博物馆画作", [], mood="comfortable")
        with tempfile.TemporaryDirectory() as tmp:
            bgm = Path(tmp) / "bgm.mp3"
            bgm.write_bytes(b"fake mp3")

            selection = self.script.build_bgm_selection("安静的博物馆画作", plan, bgm_query="", bgm_path=str(bgm))

            self.assertEqual(selection["music_source"], "local")
            self.assertEqual(selection["bgm_path"], str(bgm))
            self.assertNotIn("music_url", selection)
    ...
    def test_imported_package_script_uses_capsule_cinema_root_env(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                ...
                cwd=str(ROOT),
                env={**os.environ, "CAPSULE_CINEMA_ROOT": str(ROOT)},
                text=True,
                capture_output=True,
            )
