...
class Veo31VideoGeneratorTests(unittest.TestCase):
    ...
    def test_client_builds_first_last_frame_payload(self):
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / "output") as tmpdir:
            with patch.dict(
                os.environ,
                {
    ...
    def test_client_keeps_explicit_output_dir_under_output_root(self):
        from custom_tools.video_generation.veo31_video_generator_tool import (
            Veo31VideoClient,
        )

        with tempfile.TemporaryDirectory(dir=ROOT / "output") as tmpdir:
            output_dir = Path(tmpdir) / "veo31_videos"
            with patch.dict(
                os.environ,
                {
                    "JULING_BASE_URL": "https://example.test",
                ...
                client = Veo31VideoClient(output_dir=str(output_dir))
