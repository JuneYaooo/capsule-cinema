...
class GenerationAspectContractTest(unittest.TestCase):
    ...
    def test_gpt_image2_supports_daydream_chat_endpoint(self):
        ...
        class ImagesClientShouldNotBeUsed:
            ...
            def post(self, *_args, **_kwargs):
                raise AssertionError("images endpoint should not be used when GPT_IMAGE2_ENDPOINT=chat")

        with patch.dict(
            "os.environ",
            {
                "GPT_IMAGE2_BASE_URL": "https://daydream.example.test",
                "GPT_IMAGE2_API_KEY": "daydream-secret",
                "GPT_IMAGE2_ENDPOINT": "chat",
            ...
            result = GptImage2Tool()._run(
                prompt="水墨武则天",
                output_path=str(self.workspace / "daydream_chat.png"),
                aspect_ratio="9:16",
            )

        self.assertIn("成功", result)
        self.assertEqual(captured["url"], "https://daydream.example.test/v1/chat/completions")
        self.assertEqual(captured["kwargs"]["headers"]["Authorization"], "Bearer daydream-secret")
        self.assertEqual(captured["kwargs"]["json"]["model"], "gpt-image-2")
        self.assertEqual(captured["kwargs"]["json"]["size"], "1024x1792")
        self.assertTrue((self.workspace / "daydream_chat.png").exists())
