...
class AgnesVideoSchema(BaseModel):
    ...
    num_frames: int = Field(41, description="Requested frame count; must follow the 8n+1 rule")
...
def build_agnes_video_payload(
    *,
    ...
    resolved_frames = int(num_frames)
    if not 1 <= resolved_frames <= 441:
        raise ValueError("num_frames must be between 1 and 441")
    if (resolved_frames - 1) % 8 != 0:
        raise ValueError("num_frames must follow the Agnes 8n+1 rule")
