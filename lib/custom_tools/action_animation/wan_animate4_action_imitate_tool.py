...
# -*- coding: utf-8 -*-
"""
WanAnimate4动作模仿CrewAI工具
使用RunningHub API上传参考图片和视频，生成动作模仿视频（3piao版本）
"""
...
# 初始化日志
logger = get_logger("wan_animate4_action_imitate")


class WanAnimate4ActionImitateSchema(BaseModel):
    """WanAnimate4动作模仿工具的输入参数"""
    image_path: str = Field(
        ...,
        description="要替换的角色/人物图片路径（参考图）"
    )
    ...
    chest_shake_intensity: float = Field(
        default=0.5,
        description="胸部抖动强度（建议：0.4-0.6）"
    )
    width: int = Field(
        default=720,
        description="视频宽度"
...
class WanAnimate4ActionImitateTool(BaseTool):
    """WanAnimate4动作模仿工具"""
    name: str = "WanAnimate4动作模仿工具"
    description: str = (
        "使用WanAnimate4 API生成动作模仿视频的工具（3piao版本）。"
        "上传参考图片和视频，将图片中的角色替换到视频的动作中。"
        "支持调整帧率、胸部抖动强度和视频尺寸。"
    )
    args_schema: Type[BaseModel] = WanAnimate4ActionImitateSchema
    ...
    def _run(
        ...
        frame_rate: int = 30,
        chest_shake_intensity: float = 0.5,
        width: int = 720,
        height: int = 1280
    ) -> Dict[str, Any]:
        """
        ...
        try:
            # 初始化API客户端
            client = WanAnimate4ApiClient(output_dir=output_dir)
            ...
            video_output_path = client.action_imitate(
                ...
                chest_shake_intensity=chest_shake_intensity,
                width=width,
                height=height
            )
            ...
            if video_output_path:
                return {
                    ...
                    "status": "success",
                    "message": f"✅ WanAnimate4动作模仿成功！视频已保存到: {video_output_path}"
                }
            else:
                return {
                    "status": "failed",
                    "message": "❌ WanAnimate4动作模仿失败"
                }

        except Exception as e:
            logger.error(f"WanAnimate4动作模仿失败: {str(e)}")
            return {
                "status": "failed",
                "message": f"❌ WanAnimate4动作模仿失败: {str(e)}"
            }


class WanAnimate4ApiClient:
    """WanAnimate4 API客户端"""

    def __init__(self, output_dir: str = "output/videos"):
        """初始化API客户端"""
        self.api_key = os.getenv('WANANIMATE4_API_KEY') or os.getenv('RUNNINGHUB_API_KEY')
        if not self.api_key:
            raise ValueError("请设置环境变量 WANANIMATE4_API_KEY 或 RUNNINGHUB_API_KEY")
        self.webapp_id = os.getenv('WANANIMATE4_WEBAPP_ID', '2002895355370958850')
        ...
        payload = {
            ...
            "nodeInfoList": [
                ...
                },
                {
                    "nodeId": "375",
                    "fieldName": "value",
                    "fieldValue": str(chest_shake_intensity),
                    "description": "胸部抖动强度（建议：0.4-0.6）"
                },
                {
                    "nodeId": "259",
    ...
    def action_imitate(
        ...
        """
        logger.info(f"\n{'='*60}")
        logger.info("🎬 WanAnimate4 动作模仿 (Action Imitate - 3piao)")
        logger.info(f"{'='*60}")
        ...
        try:
            ...
            logger.info(f"   - 胸部抖动强度: {chest_shake_intensity}")
            ...
            if outputs and len(outputs) > 0:
                ...
                if file_url:
                    ...
                    if output_path:
                        ...
                        output_file = self.output_dir / f"action_imitate4_{timestamp}.mp4"
