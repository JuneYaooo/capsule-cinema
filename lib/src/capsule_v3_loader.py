...
class CapsuleV3Error(Exception):
    """Raised when a v3 capsule cannot be resolved or loaded."""


ROOT = Path(__file__).resolve().parents[2]
DEFAULT_SEARCH_ROOTS = [ROOT / "capsules_v3"]


def _read_yaml(path: Path, fallback: Any) -> Any:
    if not path.exists():
        raise CapsuleV3Error(f"missing YAML file: {path}")
    try:
        return yaml.safe_load(path.read_text(encoding="utf-8")) or fallback
    except yaml.YAMLError as exc:
        raise CapsuleV3Error(f"invalid YAML file: {path}: {exc}") from exc


def _read_text(path: Path) -> str:
    if not path.exists():
        raise CapsuleV3Error(f"missing text file: {path}")
...
def resolve_capsule_dir(
    ...
    if value.exists():
        ...
        if candidate.is_dir() and (candidate / "capsule.yaml").is_file():
            return candidate
        raise CapsuleV3Error(f"not a v3 capsule directory: {candidate}")

    name = str(name_or_path).strip()
    if not name:
        raise CapsuleV3Error("empty capsule name")
    candidates = [name]
    ...
    for root in roots:
        for candidate_name in candidates:
            ...
            if candidate.is_dir() and (candidate / "capsule.yaml").is_file():
                return candidate
    raise CapsuleV3Error(f"capsule not found: {name}")


def _load_capsule_yaml(capsule_dir: Path) -> dict[str, Any]:
    data = _read_yaml(capsule_dir / "capsule.yaml", {})
    if not isinstance(data, dict):
        raise CapsuleV3Error(f"capsule.yaml must be an object: {capsule_dir}")
    return data
...
def load_runtime_contract(
    ...
    if not isinstance(runtime, dict):
        raise CapsuleV3Error("contracts/runtime.yaml must be an object")
    return runtime
...
def _stage_files(capsule: dict[str, Any], stage: str) -> list[str]:
    ...
    if not isinstance(files, list):
        raise CapsuleV3Error(f"stage not declared in read_order: {stage}")
...
def load_quality_rules(
    ...
    if not isinstance(rules, list):
        raise CapsuleV3Error("quality/rules.yaml must contain a rules list")
...
def load_assets_index(
    ...
    if not isinstance(assets, list):
        raise CapsuleV3Error("assets/index.yaml must contain an assets list")
