...
# 初始化日志
logger = get_logger("wan_animate2_action_imitate")


class WanAnimate2ActionImitateSchema(BaseModel):
    """WanAnimate2动作模仿工具的输入参数"""
...
class WanAnimate2ActionImitateTool(BaseTool):
    """WanAnimate2动作模仿工具"""
    name: str = "WanAnimate2动作模仿工具"
    description: str = (
        "使用WanAnimate2 API生成动作模仿视频的工具。"
        "上传参考图片和视频，将图片中的角色替换到视频的动作中。"
        "适合生成人物动作迁移、舞蹈模仿等创意视频内容。"
    )
    args_schema: Type[BaseModel] = WanAnimate2ActionImitateSchema
    ...
    def _run(
        ...
        try:
            # 初始化API客户端
            client = WanAnimate2ApiClient(output_dir=output_dir)
            ...
            if video_output_path:
                return {
                    ...
                    "status": "success",
                    "message": f"✅ WanAnimate2动作模仿成功！视频已保存到: {video_output_path}"
                }
            else:
                return {
                    "status": "failed",
                    "message": "❌ WanAnimate2动作模仿失败"
                }

        except Exception as e:
            logger.error(f"WanAnimate2动作模仿失败: {str(e)}")
            return {
                "status": "failed",
                "message": f"❌ WanAnimate2动作模仿失败: {str(e)}"
            }


class WanAnimate2ApiClient:
    """WanAnimate2 API客户端"""

    def __init__(self, output_dir: str = "output/videos"):
        """初始化API客户端"""
        self.api_key = os.getenv('WANANIMATE2_API_KEY') or os.getenv('RUNNINGHUB_API_KEY')
        if not self.api_key:
            raise ValueError("请设置环境变量 WANANIMATE2_API_KEY 或 RUNNINGHUB_API_KEY")
        self.webapp_id = os.getenv('WANANIMATE2_WEBAPP_ID', '1995862612531372034')
    ...
    def action_imitate(
        ...
        """
        logger.info(f"\n{'='*60}")
        logger.info("🎬 WanAnimate2 动作模仿 (Action Imitate)")
        logger.info(f"{'='*60}")
            ...
            if outputs and len(outputs) > 0:
                ...
                if file_url:
                    ...
                    if output_path:
                        ...
                        output_file = self.output_dir / f"action_imitate2_{timestamp}.mp4"
