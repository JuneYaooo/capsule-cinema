...
# 初始化日志
logger = get_logger("wan22_animate3_action_imitate")


class Wan22Animate3ActionImitateSchema(BaseModel):
    """Wan2.2Animate3动作模仿工具的输入参数"""
...
class Wan22Animate3ActionImitateTool(BaseTool):
    """Wan2.2Animate3动作模仿工具"""
    name: str = "Wan2.2Animate3动作模仿工具"
    description: str = (
        "使用Wan2.2Animate3 API生成动作模仿视频的工具。"
        "上传参考图片和视频，将图片中的角色替换到视频的动作中。"
        "适合生成人物动作迁移、舞蹈模仿等创意视频内容。"
    )
    args_schema: Type[BaseModel] = Wan22Animate3ActionImitateSchema
    ...
    def _run(
        ...
        try:
            # 初始化API客户端
            client = Wan22Animate3ApiClient(output_dir=output_dir)
            ...
            if video_output_path:
                return {
                    ...
                    "status": "success",
                    "message": f"✅ Wan2.2Animate3动作模仿成功！视频已保存到: {video_output_path}"
                }
            else:
                return {
                    "status": "failed",
                    "message": "❌ Wan2.2Animate3动作模仿失败"
                }

        except Exception as e:
            logger.error(f"Wan2.2Animate3动作模仿失败: {str(e)}")
            return {
                "status": "failed",
                "message": f"❌ Wan2.2Animate3动作模仿失败: {str(e)}"
            }


class Wan22Animate3ApiClient:
    """Wan2.2Animate3 API客户端"""

    def __init__(self, output_dir: str = "output/videos"):
        """初始化API客户端"""
        self.api_key = os.getenv('WAN22ANIMATE3_API_KEY') or os.getenv('RUNNINGHUB_API_KEY')
        if not self.api_key:
            raise ValueError("请设置环境变量 WAN22ANIMATE3_API_KEY 或 RUNNINGHUB_API_KEY")
        self.webapp_id = os.getenv('WAN22ANIMATE3_WEBAPP_ID', '1975951975441412098')
    ...
    def create_task(
        ...
        try:
            ...
            logger.info(
                ...
                f"nodes={len(payload['nodeInfoList'])}, instance={payload.get('instanceType', 'default')}"
            )
    ...
    def action_imitate(
        ...
        """
        logger.info(f"\n{'='*60}")
        logger.info("🎬 Wan2.2Animate3 动作模仿 (Action Imitate)")
        logger.info(f"{'='*60}")
        ...
        try:
            ...
            logger.info("\n【步骤 3/5】创建动作模仿任务")
            logger.info(f"   参数设置:")
            logger.info(f"   - 宽度: {width}")
            logger.info(f"   - 高度: {height}")
            ...
            if outputs and len(outputs) > 0:
                ...
                if file_url:
                    ...
                    if output_path:
                        ...
                        output_file = self.output_dir / f"action_imitate3_{timestamp}.mp4"
