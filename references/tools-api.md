...
official Agnes free-tier image/short-text-to-video tools, official MiniMax H3
video and TTS, Doubao Speech (API Key + bidirectional WebSocket), RunningHub workflow
...
`MiniMaxH3VideoGeneratorTool` calls the official V2 create/query endpoints,
polls asynchronous tasks, and downloads the expiring MP4 locally. It supports
text, first frame, first/last frames, and multimodal image/video/audio
references. H3 V2 currently accepts only `resolution=2K`; the project does not
invent a 720p H3 mode. The default complete-video route therefore remains
Seedance at 720p, while an explicit H3/2K/high-resolution request selects
runtime engine `minimax-h3`. That route never silently downgrades to the
default lower-resolution engine.
