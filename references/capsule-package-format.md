...
`capsule_package_validate.py` (and therefore every create, update, and pack command) runs this lint automatically and fails on lint errors. Warnings are drift candidates for human review, not automatic failures. GitHub Actions re-runs the same validation on every push, so a drifted capsule cannot enter the public repository unnoticed even when the writing agent skipped every instruction.
